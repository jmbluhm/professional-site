...
const writingArticles = [
  {
    title: 'I asked ChatGPT what it\'s like to work with me',
    url: '/content/chatgpt-perspective',
  },
  {
          ...
          {writingArticles.map((article) => (
            <div key={article.url}>
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className={`flex items-center gap-2 text-zinc-900 dark:text-zinc-100 hover:text-zinc-600 dark:hover:text-zinc-400 ${article.title === 'I asked ChatGPT what it\'s like to work with me' ? 'font-medium' : ''}`}
                style={article.title === 'I asked ChatGPT what it\'s like to work with me' ? { color: '#D97757' } : undefined}
              >
                <svg
                  className="w-4 h-4 flex-shrink-0"
                  fill="currentColor"
